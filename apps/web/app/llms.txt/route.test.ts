...
describe("GET /llms.txt", () => {
  ...
  // `renderLlmsTxt` entirely (already covered against real story content by
  // `lib/llms/generate-llms.test.ts`), so this route's own guard exercises
  // the real `renderLlmsTxt` here, wired through the real GET() handler,
  // proving the route itself introduces no leak of its own.
  it("serves the real renderLlmsTxt output, which contains no story sentence or title (#296)", async () => {
    const real = await vi.importActual<typeof import("../../lib/llms/generate-llms")>(
      "../../lib/llms/generate-llms",
    );
    renderLlmsTxt.mockImplementation(real.renderLlmsTxt);
    getSiteUrl.mockReturnValue("https://stub-deploy.example.com");
    getMcpEndpointUrl.mockReturnValue("https://stub-deploy.example.com/api/mcp");
    const { GET } = await import("./route.js");
