...
describe("computeReadmeRegions (#23 — README.md generated regions)", () => {
  it("produces one region per marked section README.md defines", () => {
  ...
  it("renders one bullet per example prompt, derived from the real tool catalogue", () => {
    const regions = computeReadmeRegions(ENDPOINT_URL);
    const region = regions.find((r) => r.id === "mcp-example-prompts");
