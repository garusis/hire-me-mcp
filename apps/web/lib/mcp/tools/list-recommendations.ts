/**
 * `list-recommendations` — thin adapter over `packages/core`'s
 * `listRecommendations()` (#190). No business logic: this handler does not
 * filter, re-rank, summarize, or reword anything the domain service
 * returns (#31). Read-only scope: display + link-out only.
 */
...
/**
 * Derived from `listRecommendations`'s own return type rather than
 * importing `Recommendation` from `@hire-me-mcp/career-data` directly —
 * apps/web reads career content only through `src/lib/content/` (issue
 * #16, enforced by biome).
 */
