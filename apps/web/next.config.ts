...
const nextConfig: NextConfig = {
  ...
  // #35 — the downloadable CV PDF is a plain static asset under
  // public/cv/<deterministic-filename>.pdf (regenerated by `pnpm
  // generate:cv`, wired into `@hire-me-mcp/web#build` in turbo.json, never
  // committed). `Content-Disposition: attachment` with no `filename`
  // parameter tells the browser to save-as using the request URL's own
  // last path segment — which is already the deterministic,
  // profile-name-derived filename `getCvView()` computed — so this stays
  // correct without duplicating that filename logic here. Next's
  // `headers()` applies to files served from `public/` the same as any
  // other route, so this needs no custom route handler or filesystem
  // tracing.
  async headers() {
    return [
      {
