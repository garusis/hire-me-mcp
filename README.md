...
- **Honest, investigated CI limitation**: Google rejects this free-tier key's calls specifically from GitHub Actions runner IPs — the exact same key is fingerprint-verified as delivered correctly, works locally, and works from Vercel (production chat, `preview-e2e`'s own chat specs). A one-token pre-check probes this before the real dataset runs; when Google rejects it, the job concludes as an explicit, loud SKIP (green, not red) instead of staying red indefinitely for an environmental reason no workflow change can fix. Full evidence writeup in `packages/agent/README.md`'s "Known constraint" section — the actual operative gate today is the documented local `pnpm eval:agent` run before a release, plus the v1.0 certification pass (#76).
